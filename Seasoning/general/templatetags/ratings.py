...
def rating_display_stars(rating):
    if rating is None:
        return ''
    width_percentage = round((rating/5.0)*100, 1)
...
def mean_rating(rating):
    if rating is None:
        return '<span id="account-mean-rank-off" style="width: 100%"></span>'
    width_percentage = round((rating/5.0)*100, 1)
...
def mean_footprint(footprint):
    ...
    width_percentage = round((footprint/Recipe.objects.all().aggregate(Max('footprint'))['footprint__max'])*100, 1)
